//! Writes golden fixtures for `loomem_core::persisted_codec`, encoded by the
//! reference implementation (`bincode` 1.3.3, default options) rather than by
//! the codec under test. The output describes the *current* checkout of
//! `loomem-core` (recorded as `source_sha`) and the compiler that ran the
//! tool (recorded as `rustc`); only the committed v1 file carries the
//! historical "written by the pre-codec engine" provenance.
//!
...
fn main() -> Result<()> {
    ...
    let doc = json!({
        "_comment": format!(
            "Golden fixtures for loomem-core/src/persisted_codec.rs: reference bincode 1.3.3 output \
             (bincode::serialize, default options) for the loomem-core source at {source_sha}, \
             produced by tools/legacy-fixture-gen with {rustc} on {generated_at}. All keys are \
             synthetic. The v1 file is the historical record written by the pre-codec engine and is \
             never regenerated; see fixtures/legacy_bincode_v1.md."
        ),
        "source_sha": source_sha,
        "generated_at": generated_at,
        "generator": "tools/legacy-fixture-gen",
        "rustc": rustc,
